...
with h5py.File(file_out + ".h5", "w") as f:
    ...
    train_data.create_dataset("x1", data=x1)
    train_data.create_dataset("x2", data=x2)
    train_data.create_dataset("y1", data=y1)
    train_data.create_dataset("y2", data=y2)

    # GP predictions
    predictions = f.create_group("predictions")
    predictions.create_dataset("mean1", data=gp_mean1_lst)
    predictions.create_dataset("mean2", data=gp_mean2_lst)
    predictions.create_dataset("std1", data=gp_std1_lst)
    predictions.create_dataset("std2", data=gp_std2_lst)
