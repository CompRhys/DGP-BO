...
for j in range(rep):
    ...
    GPR_m1=gp_model(x_init_m1, y_init_m1.reshape(x_init_m1.shape[0]), initial_l, sf_m1, sn_m, N_dim, 'SE' , mean=0)
    GPR_m2=gp_model(x_init_m2, y_init_m2.reshape(x_init_m2.shape[0]), initial_l, sf_m2, sn_m, N_dim, 'SE' , mean=0)
    GPR_m3=gp_model(x_init_m3, y_init_m3.reshape(x_init_m3.shape[0]), initial_l, sf_m3, sn_m, N_dim, 'SE' , mean=0)
    ...
    y=np.concatenate((y1.reshape(100,1),y2.reshape(100,1),y3.reshape(100,1)),axis=1)
    ...
    while iteration<itr:
        ...
        pd.DataFrame(y_pareto_truth).to_csv("/scratch/user/danialkh26/myMTGP_rec/y_pareto_truth.csv", header=None, index=None)
        ...
        pd.DataFrame(data_x).to_csv("/scratch/user/danialkh26/myMTGP_rec/data_x.csv", header=None, index=None)
        pd.DataFrame(data_y).to_csv("/scratch/user/danialkh26/myMTGP_rec/data_y.csv", header=None, index=None)
        pd.DataFrame(hv_truth).to_csv("/scratch/user/danialkh26/myMTGP_rec/hv_truth.csv", header=None, index=None)
        
        
    hv_total.append(np.ravel(hv_truth))
    pd.DataFrame(hv_total).to_csv("/scratch/user/danialkh26/myMTGP_rec/hv_truth_total.csv", header=None, index=None)
