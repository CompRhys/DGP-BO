...
#         )
        self.covar_module = gpytorch.kernels.ScaleKernel(
            gpytorch.kernels.RBFKernel(batch_shape=torch.Size([10])), #,lengthscale_constraint=gpytorch.constraints.Interval(0.2, 0.8)
            batch_shape=torch.Size([10])
        )
...
class DGPHiddenLayer(DeepGPLayer):
    def __init__(self, input_dims, output_dims, num_inducing=128, linear_mean=True):
        ...
        self.covar_module = ScaleKernel(
            MaternKernel(nu=2.5, batch_shape=batch_shape, ard_num_dims=input_dims), #,lengthscale_constraint=gpytorch.constraints.Interval(0.2, 0.8)
            batch_shape=batch_shape, ard_num_dims=None
        )
...
#         )
        ...
        self.likelihood = gpytorch.likelihoods.GaussianLikelihood(num_tasks=3,noise_constraint=gpytorch.constraints.Interval(0.001, 10)) #,noise_constraint=gpytorch.constraints.Interval(0.001, 10)
...
#         print(task_indices)
        ...
        task_indices1=torch.from_numpy(np.ones(inputs.shape[0],dtype=int)*1).long().cuda()*task_indices
...
#         print(torch.distributions.Normal(loc=hidden_rep1.mean, scale=hidden_rep1.variance.sqrt()).rsample().shape,"sample shape")
        output=self.last_layer(torch.distributions.Normal(loc=hidden_rep1.mean, scale=hidden_rep1.variance.sqrt()).rsample(),task_indices=task_indices1.cuda())
#         print(output)
        return output
...
from acquisitionFuncdebug2 import expected_improvement
train_x1 = torch.from_numpy((dirlet5D(2,5))/32).cuda()#[np.random.randint(0,557,size=3),:]
train_x2 = train_x1
train_x3 = train_x1
...
mll = DeepApproximateMLL(VariationalELBO(model.likelihood, model, num_data=full_train_y.size(0)))
...
yp=np.concatenate((train_y1t.reshape(train_y1t.shape[0],1),train_y2t.reshape(train_y2t.shape[0],1)),axis=1)
...
#             print(i)
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            if i == 0:
                ...
                test_i_task1 = torch.full((test_xt.shape[0],1), dtype=torch.long, fill_value=0).cuda()
                test_i_task2 = torch.full((test_xt.shape[0],1), dtype=torch.long, fill_value=1).cuda()
                test_i_task3 = torch.full((test_xt.shape[0],1), dtype=torch.long, fill_value=2).cuda()

                observed_pred_y1 = model.likelihood(model(test_xt.float(),task_indices= test_i_task1.squeeze(1)))
                observed_pred_y2 = model.likelihood(model(test_xt.float(),task_indices= test_i_task2.squeeze(1)))
                observed_pred_y3 = model.likelihood(model(test_xt.float(),task_indices= test_i_task3.squeeze(1)))
...
#         print(test_xt.float())
    ...
    if (k%3)==0:
        task=0
        eh_mean=np.concatenate((np.array([mean_t1.cpu().numpy()]).T,np.array([mean_t2.cpu().numpy()]).T),axis=1)
        eh_std=np.concatenate((np.array([var_t1.cpu().numpy()]).T,np.array([var_t2.cpu().numpy()]).T),axis=1)
...
#             test_x = torch.cat((test_x[:x_star],test_x[x_star+1:]))
    ...
    #     new_y= torch.cos(new_x * (2 * math.pi))# + torch.randn(new_x.size()) * 0.2
        ...
        data_x=np.concatenate((train_x2.detach().cpu().numpy(),np.array([new_x.cpu().numpy()])),axis=0)
        data_y=np.concatenate((train_y2.detach().cpu().numpy(),new_y2.cpu().numpy().reshape(1)),axis=0)
        train_x2=torch.tensor(data_x).cuda()
        train_y2=torch.tensor(data_y).cuda()
        data_x=np.concatenate((train_x3.detach().cpu().numpy(),np.array([new_x.cpu().numpy()])),axis=0)
        data_y=np.concatenate((train_y3.detach().cpu().numpy(),new_y3.cpu().numpy().reshape(1)),axis=0)
        train_x3=torch.tensor(data_x).cuda()
        train_y3=torch.tensor(data_y).cuda()

        data_y=np.concatenate((train_y1t.detach().cpu().numpy(),new_y.cpu().numpy().reshape(1)),axis=0)

        train_y1t=torch.tensor(data_y).cuda()

        data_y=np.concatenate((train_y2t.detach().cpu().numpy(),new_y2.cpu().numpy().reshape(1)),axis=0)
        train_y2t=torch.tensor(data_y).cuda()

        yp=np.concatenate((train_y1t.cpu().numpy().reshape(train_y1t.cpu().numpy().shape[0],1),train_y2t.cpu().numpy().reshape(train_y2t.cpu().numpy().shape[0],1)),axis=1)
...
#         max_val, x_star, EI = expected_improvement(np.max(train_y1.detach().cpu().numpy()), xi,mean_t.cpu().numpy(), var_t.sqrt().cpu().numpy())
        max_val2, x_star2, UCB2 =upper_conf_bound(kt ,mean_t2.cpu().numpy(), var_t2.cpu().numpy())#.sqrt()
        max_val3, x_star3, UCB3 =upper_conf_bound(kt ,mean_t3.cpu().numpy(), var_t3.cpu().numpy())#.sqrt()
...
#         train_x1=torch.tensor(data_x).cuda()
#         train_y1=torch.tensor(data_y).cuda()
        data_x=np.concatenate((train_x2.detach().cpu().numpy(),np.array([new_x2.cpu().numpy()])),axis=0)
        data_y=np.concatenate((train_y2.detach().cpu().numpy(),new_y2.cpu().numpy().reshape(1)),axis=0)
        train_x2=torch.tensor(data_x).cuda()
        train_y2=torch.tensor(data_y).cuda()
        data_x=np.concatenate((train_x3.detach().cpu().numpy(),np.array([new_x3.cpu().numpy()])),axis=0)
        data_y=np.concatenate((train_y3.detach().cpu().numpy(),new_y3.cpu().numpy().reshape(1)),axis=0)
        train_x3=torch.tensor(data_x).cuda()
        train_y3=torch.tensor(data_y).cuda()
...
#     full_train_y = torch.cat([train_y1, train_y2,train_y3,train_y4])
    ...
    model.train()
    optimizer = torch.optim.Adam(model.parameters(), lr=0.1)
    mll = DeepApproximateMLL(VariationalELBO(likelihood, model, num_data=train_y1.size(0)))
    if (counter < 250)==True:
        num_epochs = 500  
    else:
        num_epochs = 250
