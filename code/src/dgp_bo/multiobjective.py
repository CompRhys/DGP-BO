...
def EHVI(mm, ss, goal, ref, pareto):
    ...
    for i in range(means.shape[0]):
        ...
        for j in range(N_obj):
            ...
            box = box * (
                (ref[0, j] - means[i, j]) * norm.cdf(s) + sigmas[i, j] * norm.pdf(s)
            )

        # calculate how much adding a test point can improve the hypervolume
        hvi = recursive(
            means[i, :].reshape(1, N_obj), sigmas[i, :].reshape(1, N_obj), ref, pareto
        )
...
def recursive(means, sigmas, ref, pareto):
    ...
    if pareto.shape[0] == 1:
        ...
        for j in range(N_obj):
            ...
            up = (ref[0, j] - means[0, j]) * norm.cdf(s_up) + sigmas[0, j] * norm.pdf(
                s_up
            )
            low = (pareto[0, j] - means[0, j]) * norm.cdf(s_low) + sigmas[
                0, j
            ] * norm.pdf(s_low)
