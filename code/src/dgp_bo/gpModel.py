...
class gp_model:
    ...
    def predict_cov(self, x_pred):
        ...
        mean, sigma = self.gp.predict(
            self.y_train, x_pred, kernel=self.kk, return_cov=True, return_var=False
        )
        return mean, sigma
    ...
    def predict_var(self, x_pred):
        ...
        mean, var = self.gp.predict(
            self.y_train, x_pred, kernel=self.kk, return_cov=False, return_var=True
        )
        return mean, var

    def update(self, new_x_data, new_y_data, new_y_err):
        # This function is used to update and retrain the GP model when new
        # training data is available
        self.x_train = np.append(self.x_train, new_x_data, axis=0)
        self.y_train = np.append(self.y_train, new_y_data, axis=0)
    ...
    def sample_posterior(self, x_test, N_samp):
        ...
        return self.gp.sample_conditional(self.y_train, x_test, size=N_samp)
