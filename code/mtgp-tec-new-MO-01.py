...
class MultitaskGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        ...
        self.covar_module = gpytorch.kernels.MaternKernel(lengthscale_constraint=gpytorch.constraints.Interval(0.2, 0.8)) #lengthscale_constraint=gpytorch.constraints.Interval(0.01, 0.1)
#MaternKernel
        # We learn an IndexKernel for 2 tasks
        # (so we'll actually learn 2x2=4 tasks with correlations)
        self.task_covar_module = gpytorch.kernels.IndexKernel(num_tasks=3, rank=3) #gpytorch.constraints.Interval(-1, 1)),var_constraint=gpytorch.constraints.GreaterThan(-0.5)
...
likelihood = gpytorch.likelihoods.GaussianLikelihood(noise_constraint=gpytorch.constraints.Interval(0.001, 10)) #noise_constraint=gpytorch.constraints.Interval(1, 5)
